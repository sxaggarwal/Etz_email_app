...
class EmailGui(tk.Tk):
    ...
    def make_combobox(self):
        ...
        send_mail_button = tk.Button(self, text="Send Email", command=lambda: self.verify_and_send_email(self.rfq_number.get(), other_attachment=list(self.other_attachments.get(0, tk.END))))
        send_mail_button.grid(row=5, column=0)

    def verify_and_send_email(self, rfq_number, other_attachment=[]):
        if rfq_number:
            send_mail(rfq_number, other_attachment=other_attachment)
